				...
				<div class="row">
					<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
						...
						<c:forEach var="v" items="${bList}">
							<tr>
								<td>${v.b_no}</td>
								<td style="text-align: left"><a href="boardViewOneAction.do?pageNum=${pageNum}&b_no=${v.b_no}" style= "color:black;">${v.b_title}</a></td>
								<td>${v.b_writer}</td>
								<td>${v.b_date}</td>
								<td>${v.b_readcount}</td>
							</tr>
                    ...
                    <tr>
                        ...
                        <c:if test="${printPage[0] > 10}">
                            <a href="boardSearchAction.do?search=${search}&pageNum=${printPage[0]-10}" style="color:black"><b>이전</b></a>&nbsp;&nbsp;
                        </c:if>                
                        <c:forEach var="p" items="${printPage}">
                            <c:if test="${p > 0}">
                                ...
                                <c:if test="${p != pageNum}">
                                    <a style="color: gray" href="boardSearchAction.do?search=${search}&pageNum=${p}" style="color:black">${p}</a>&nbsp;&nbsp;
                                </c:if>
                            </c:if>
                        </c:forEach>
                        <c:if test="${printPage[0]+10 < totalPage}">
                            <a href="boardSearchAction.do?search=${search}&pageNum=${printPage[0]+10}" style="color:black"><b>다음</b></a>&nbsp;&nbsp;
                        </c:if>
                        <%-- 페이지처리 끝 --%>
                        </td>
                        <td>
